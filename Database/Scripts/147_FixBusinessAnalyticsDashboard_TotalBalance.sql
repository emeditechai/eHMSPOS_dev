...
-- ============================================================
-- Script  : 147_FixBusinessAnalyticsDashboard_TotalBalance.sql
-- Purpose : TotalBalance in sp_GetBusinessAnalyticsDashboard
--           was using SUM(b.BalanceAmount) which includes payments
--           from all billing heads (Stay + Other Charges + Room Services).
--           Since TotalAmount on Bookings = Stay bill only, we compute
--           TotalBalance as TotalAmount minus Stay-only payments
--           (BillingHead = 'S' or NULL) so Other Charges payments
--           do not make the balance appear negative.
--           No other logic in the SP is changed.
-- ============================================================
...
CREATE PROCEDURE [dbo].[sp_GetBusinessAnalyticsDashboard]
    @BranchID INT,
    @FromDate DATE,
    @ToDate DATE
AS
BEGIN
    ...
    SELECT
        ...
        -- FIX: TotalBalance computed from TotalAmount minus Stay-only payments.
        -- Other Charges (BillingHead='O') and Room Services payments are separate
        -- billing heads and must not reduce the Stay outstanding balance.
        -- OUTER APPLY used to avoid nested aggregate restriction.
        ISNULL(
            (
                SELECT SUM(b.TotalAmount - ISNULL(sp.StayPaid, 0))
                FROM dbo.Bookings b
                INNER JOIN #BookingsInRange bir ON bir.Id = b.Id
                OUTER APPLY (
                    SELECT SUM(bp2.Amount + ISNULL(bp2.DiscountAmount, 0)) AS StayPaid
                    FROM   dbo.BookingPayments bp2
                    WHERE  bp2.BookingId = b.Id
                      AND  (bp2.BillingHead = 'S' OR bp2.BillingHead IS NULL)
                ) sp
            ), 0
        ) AS TotalBalance;

    -- B) Daily Trend (unchanged)
    SELECT
    ...
    -- C) Payment Method Summary (unchanged)
    SELECT
...
PRINT 'sp_GetBusinessAnalyticsDashboard updated: TotalBalance now uses Stay-only payments';
GO
