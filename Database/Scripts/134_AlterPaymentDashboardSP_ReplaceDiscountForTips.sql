-- Script 134: Replace TotalTips with TotalDiscount in sp_GetPaymentDashboardData
-- TotalDiscount = SUM of bp.DiscountAmount (discount given at payment time, per payment row).
USE HMS_Dev;
GO
...
BEGIN
    ...
    SELECT
        ...
        ISNULL(SUM(CASE WHEN bp.Status IN (''Completed'',''Captured'',''Success'')
            THEN ISNULL(bp.DiscountAmount, 0) ELSE 0 END), 0) AS TotalDiscount,
...
PRINT 'sp_GetPaymentDashboardData updated: TotalTips replaced with TotalDiscount';
GO
