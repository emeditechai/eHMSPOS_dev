-- =============================================
-- Migration 108: Add user-visibility filter to sp_GetDailyCollectionRegister
--
-- Why:
--  - Administrator and Manager roles see all data for their branch.
--  - All other roles see only records they created (bp.CreatedBy = @UserId).
--
-- Changes:
--  - Added @UserId INT and @IsAdmin BIT parameters to the stored procedure.
--  - Added AND (@IsAdmin = 1 OR bp.CreatedBy = @UserId) filter to all
--    three result sets: Summary, Daily Totals, and Details.
-- =============================================
...
CREATE PROCEDURE sp_GetDailyCollectionRegister
    ...
    @ToDate   DATE,
    @UserId   INT  = 0,
    @IsAdmin  BIT  = 1   -- default 1 so old callers without the param still see all data
AS
BEGIN
    SET NOCOUNT ON;
    ...
    DECLARE @SqlSummary NVARCHAR(MAX) = N'
        ...
        WHERE b.BranchID = @BranchID
          ...
          AND (@IsAdmin = 1 OR bp.CreatedBy = @UserId);';

    EXEC sp_executesql
        @SqlSummary,
        N'@BranchID INT, @FromDate DATE, @ToDate DATE, @UserId INT, @IsAdmin BIT',
        @BranchID = @BranchID,
        @FromDate  = @FromDate,
        @ToDate    = @ToDate,
        @UserId    = @UserId,
...
PRINT 'Stored Procedure sp_GetDailyCollectionRegister updated with user-visibility filter (Migration 108)';
GO
